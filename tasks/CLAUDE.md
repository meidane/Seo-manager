...
«همه‌ی همکاران» که `assignee=` خالی می‌فرستد)، کاربر `own_tasks_only` هم نداشت **و
...
محدودیتِ امنیتی — `own_tasks_only` (سختگیرانه، همیشه فعال، برنده‌ی هرچیزِ دیگر) جداست.
برای دیدنِ همه: دراپ‌داون → «همه‌ی همکاران».
**تلهٔ رفع‌شده:** این پیش‌فرض اولش بی‌قیدوشرط بود — یعنی «سرپرست»/کسی با `review`/
`manage_colleagues`/`manage_projects` هم با بازِ اولِ صفحه فقط تسکِ خودش را می‌دید و
...
- **تسکِ زیرمجموعه:** اگر `my_colleague.reports.exists()` (کسی مدیرشِ همین کاربر است —
  ...
  `has_reports` یا `m.can('review'/'manage_colleagues'/'manage_projects')` — یعنی
  سرپرست‌بودن نیازی به `manage_projects` (که سازمانی/همه‌ی‌پروژه‌هاست) ندارد؛ داشتنِ
...
`api.can_manage_any_timer(m)` تأیید کند — `edit_time` **یا** `edit_task`-بدونِ-
`own_tasks_only`. **تله‌ی رفع‌شده:** اول فقط `edit_time` اجازه می‌داد، یعنی مدیری با
`edit_task`+`manage_projects` (بدونِ تیک‌زدنِ جداگانه‌ی `edit_time`) اصلاً دکمه‌ی پلیِ
