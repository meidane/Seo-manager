...
- **`Report.content_cost()`** = فقط آیتم‌های سطلِ `publish`/`update`: `time`=Σ(ساعتِ تخمین×نرخِ
  ساعتی)، `word`=Σ(تعدادِ کلمه×نرخِ کلمه)، `total`=جمع. `ReportItem.eff_word_count`/`eff_estimate`.
- **ردیفِ خودکارِ فاکتور:** با اتصالِ فاکتور به گزارش (دراپ‌داونِ `report_update` یا «＋ فاکتور
  جدید» → `invoice_create`)، `content_cost.ensure_content_invoice_line(report)` یک‌بار یک
  ردیفِ «تولید محتوا (خودکار) — گزارش #id» به فاکتور می‌افزاید (اگر total>۰ و تکراری نباشد).
  چون ردیفِ فاکتور رکوردِ واقعی است، اسنپ‌شات می‌ماند.
