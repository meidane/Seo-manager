...
## TODO (فازِ بعد — خواستِ کاربر، هنوز پیاده نشده)
- **هزینهٔ تولیدِ محتوا (اسنپ‌شات):** فیلدِ پروژه «هزینه تولید محتوا» با دو زیرفیلد
  (نرخِ ساعتی، نرخِ هر کلمه)؛ محاسبهٔ `estimate_minutes×نرخ` و `word_count×نرخ` برای
  سکشن‌های تولید/آپدیت؛ وقتی فاکتور به گزارش وصل شد یک ردیفِ «تولید محتوا» خودکار (۰ =
  محاسبه نشود). **باید اسنپ‌شات شود** — تغییرِ بعدیِ نرخ نباید فاکتور/گزارشِ قبلی را عوض کند.
- **افزودن سکشن سفارشی** به گزارش (عنوان + توضیحات) با نامِ «افزودن سکشن و نمودار».
- **بازطراحیِ فیلدهای ساختِ گزارش** مثلِ نمایش + یک ستونِ «تاریخِ انجام»، همه ادیت‌پذیر،
  توضیحات ۲–۳ خط بعد اسکرول.
- **سکشنِ جایگاهِ کلماتِ کلیدی** (بالای گزارش، بعدِ توضیحات؛ فعلاً فقط ستونِ «کلمهٔ کلیدی»
  + «جایگاه»). **کانکتِ آینده (خواستِ صریحِ کاربر — اینجا مستند شد):** وقتی سراغِ بخشِ
  کلماتِ کلیدیِ track رفتیم، **همه‌چی باید بهم وصل شود** — جایگاه‌های داخلِ گزارش ↔ تسک‌ها
  ↔ رپورتاژ ↔ افزونهٔ کروم (`seo/api.py`) — با **هیستوریِ** جایگاه (منبعِ واحد در `seo/`؛
  `seo/signals.py: sync_tracked_keywords` و `seo/rank.py` نقاطِ اتصالِ فعلی‌اند).
