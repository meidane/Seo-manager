...
def _save_lines(invoice, lines):
    ...
    محافظِ ضدِ نابودیِ داده: اگر ورودی **هیچ ردیفِ معتبری** ندارد ولی فاکتور از قبل
    ردیف دارد، حذف‌وبازساخت انجام **نمی‌شود** (به‌جای پاک‌کردنِ همه‌چیز). این جلوی
    باگِ فرانت/سابمیتِ خالی را می‌گیرد که کلِ ردیف‌های یک فاکتورِ واقعی را نابود کند
    (خواستِ کاربر: «به هیچ عنوان اطلاعات نپرد»). حذفِ آگاهانه‌ی همه‌ی ردیف‌ها از راهِ
    حذفِ خودِ فاکتور انجام می‌شود، نه ذخیره‌ی خالی."""
    def _valid(li):
        return bool(li.get('description') or li.get('category') or
                    parse_amount(li.get('unit_price', 0)))
    incoming_valid = [li for li in (lines or []) if _valid(li)]
    if not incoming_valid and invoice.lines.exists():
        return False   # هیچ ردیفِ معتبری نیامد ولی فاکتور ردیف دارد → دست نزن
