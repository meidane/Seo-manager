...
2. **رنگ فقط از متغیر CSS.** هیچ رنگ hard-code در CSS. **این قانون همان چیزی است که
   تمِ روشن را بدونِ نگه‌داریِ دو نسخه ممکن می‌کند:** تمِ روشن فقط یک بلوکِ
   `:root[data-theme="light"]` است که **مقدارِ متغیرها** را عوض می‌کند — هیچ قاعدهٔ
   کامپوننتیِ جداگانه‌ای ندارد. پس هر کامپوننتِ جدید که فقط از متغیر رنگ بگیرد، خودکار در
   هر دو تم درست کار می‌کند. پُرکننده‌های نیمه‌شفافِ سطحی هم متغیر دارند: `--fill`/
   `--fill-strong`/`--row-alt`/`--scrollbar` (به‌جای `rgba(255,255,255,.0x)`ِ خام). سوییچ:
   `#theme-toggle` سایدبار + رادیوهای `data-theme-radio` تنظیمات → `App.setTheme` (ذخیره در
   `localStorage`، اعمالِ قبل‌از‌رنگ‌آمیزی در `<head>`ِ `base.html`). پیش‌فرض تیره.
...
29. **`<input>`ِ لختِ داخلِ ردیفِ فلکس با `width:Npx`ِ کلاسِ خودت، به‌خاطرِ قاعدهٔ سراسریِ
    `input[type=text]{width:100%}` (خط ۲۵۲ style.css) پهن می‌شود و بقیهٔ فلکس‌آیتم‌ها را
    له می‌کند (بیت‌مان زد: اینپوتِ تخمینِ شخصی به ۵۰۹px کشید و عنوانِ تسک به عرضِ ۰ رفت →
    نامرئی).** انتخاب‌گرِ عنصر+صفت `input[type=text]` (0,1,1) از کلاسِ تنها (`.x`، 0,1,0)
    خاص‌تر است. رفع: انتخاب‌گر را خاص‌تر کن (`.wrap .x` یا `input.x`) **یا** به‌جای اینپوتِ
    خام از ماژولِ آماده‌ی همان کار استفاده کن. **درسِ بزرگ‌تر:** به‌جای ساختنِ کنترلِ نو،
    ماژولِ سراسریِ موجود را عیناً به‌کار ببر تا ظاهر/رفتار با «بقیهٔ جاها» یکی شود — مثلاً
    پلی/تایمر/تخمینِ فضای شخصی از همان `.timer-cell`‌ی می‌آید که `_rows.html`/`tasks.js`
    دارند، نه دکمهٔ دست‌سازِ جدید. **و هر تغییرِ UI را در مرورگر (Playwright) واقعاً باز کن
    و ببین** — `manage.py check` و رندرِ ۲۰۰ کافی نیست، این باگ فقط با نگاه به صفحه پیدا شد.
30. **`position:sticky`ِ هدرِ جدول نسبت به اسکرولِ صفحه کار نمی‌کند اگر هر جدِّ بالادست
    یکی از این‌ها را داشته باشد: `overflow` غیرِ visible (مثلِ `.card{overflow:hidden}` یا
    رَپِ `.tscroll{overflow-x:auto}` که `overflow-y` را هم به auto می‌بَرد)، **یا**
    `backdrop-filter`/`filter`/`transform` (مثلِ `.glass`).** هرکدام یک scroll/containing
    context می‌سازد و sticky را از viewport جدا می‌کند. رفعِ هدرِ جدولِ پروژه‌ها: روی
    دسکتاپ (که جدول کامل جا می‌شود، پس اسکرولِ افقی لازم نیست) رَپ و کارت را
    `overflow:visible` و `backdrop-filter:none` کردیم (`@media(min-width:1101px)`،
    `.proj-card/.proj-scroll`)؛ روی موبایل اسکرولِ افقی می‌ماند و هدرِ چسبان قربانی می‌شود.
    **تلهٔ اندازه‌گیری:** برای تستِ sticky باید تا جایی اسکرول کنی که هدر واقعاً به بالای
    viewport برسد (بیشتر از ارتفاعِ بالای جدول)، وگرنه هدر هنوز طبیعی پایین‌تر است و
    «نچسبیده» به‌نظر می‌رسد در حالی که درست است.

## انضباط نگه‌داری (مهم)
**به‌روزرسانی هینت بخشی از همان تغییر است.** وقتی فیلد/الگو/تله/منبع‌واحدِ جدید اضافه شد،
در همان کامیت `CLAUDE.md`/`RECIPES` مربوط را به‌روز کن.
**گزارشِ حذف (`docs/CLEANUP.md`):** هر وقت تغییری یک جدول/ستون/بخش را **بلااستفاده** کرد،
همان‌جا ثبتش کن تا کاربر دستورِ حذف بدهد. **drop ستون/جدول فقط با تأییدِ کاربر** (روی
پنل دیتای واقعی هست؛ حذفِ ستون دادهٔ آن ستون را از بین می‌برد). **Definition of Done هر تغییر:**
