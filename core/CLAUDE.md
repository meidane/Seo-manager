...
### به‌روزرسانیِ زنده بدونِ رفرش (پولرِ واحدِ ۶۰ثانیه‌ای)
**یک پولرِ سراسری بس است — همان پولِ ۶۰ثانیه‌ایِ زنگوله.** پولِ دیگری نساز.
- `notifications_api` علاوه بر `unread`/`items`، `review_count` را هم می‌دهد
  (منبعِ واحد: `tasks/queries.py: review_pending_count(request)` — context processor
  و این API هر دو از همین می‌خوانند).
- پولِ هدر هر ۶۰ثانیه: (۱) بَجِ زنگوله، (۲) بَجِ زندهٔ «بازبینی»ی سایدبار
  (`#review-badge`، همیشه رندر می‌شود، `hidden` وقتی صفر)، (۳) **توستِ اعلانِ تازه‌رسیده**
  (نسبت به `lastMaxId` که از `#notif-list[data-max-id]` هنگامِ بارگذاری seed می‌شود — تا
  اعلانِ رسیده *بعد از* باز شدنِ صفحه توست شود، نه باز-پایه). سپس رویدادِ `window`ِ
  **`notif-poll`** با `{unread, review, fresh}` می‌فرستد.
- **چیپِ «به‌روزرسانی»** (`components/_live_refresh.html`): در صفحاتی که لیستشان باید زنده
  تازه شود include می‌شود (فعلاً `tasks/list.html` + `tasks/review.html`). به `notif-poll`
  گوش می‌دهد؛ با `fresh>0` یک چیپِ بالای صفحه («N موردِ جدید — نمایش») نشان می‌دهد که
  کلیک=رفرش. رفرشِ خودکار عمداً نیست (کاربر ممکن است وسطِ ویرایشِ inline باشد).
- نتیجه: مدیر بدونِ رفرش می‌فهمد تسکی به بازبینی رفت (توست + بَجِ سایدبار) و کارمند
  می‌فهمد تسکش برگشت (توستِ needs_fix + چیپِ لیست) — چون `task_review`/`_notify_pending_review`
  از قبل این اعلان‌ها را می‌سازند؛ فقط لایهٔ نمایشِ زنده اضافه شد.
