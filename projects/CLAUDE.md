...
## تلهٔ جدیِ رفع‌شده: شمارشِ چندبرابرِ تسک در لیست (JOINِ چندرابطه‌ای)
`ProjectListView.get_queryset` قبلاً در **یک** `annotate` هم aggregateهای رابطهٔ `tasks`
(`Count/Sum`) و هم `Max('reports__…')` و `Max('transactions__…')` را داشت. جمعِ چند رابطهٔ
چندمقداری در یک annotate باعثِ **ضربِ ردیف‌ها** (کارتزین) می‌شود: مثلاً `planned` واقعی ۲۰،
ولی با ۴ تراکنش می‌شد ۸۰ و با گزارش‌ها ۱۳۰/۶۳۹. **قاعده:** در یک annotate فقط روی **یک**
رابطهٔ چندمقداری aggregate بگیر. `last_report`/`last_payment` حالا جدا در
`get_context_data` با دو کوئریِ گروهیِ سبک محاسبه و روی هر `p` ست می‌شوند (مثلِ spark).
`Count(distinct=True)` فقط شمارش را درست می‌کند نه `Sum` را — پس راهِ درست جداکردنِ رابطه‌هاست.

## اسپارک‌لاینِ روند = LTR (امروز سمتِ راست)
`.spark{direction:ltr}` — آرایهٔ spark قدیمی→جدید ساخته می‌شود؛ بدونِ `ltr` در صفحهٔ RTL
بارها برعکس می‌شدند و «امروز» جای روزِ ۱۴روزِ پیش می‌افتاد (کاربر گزارش داد «امروز کاری
نکردم ولی نمودار ۱ نشان می‌دهد»). حالا قدیمی‌ترین چپ، امروز راست، هماهنگ با tooltip.

## لوگوی بزرگ بالای سینگلِ پروژه
هدرِ `projects/detail.html` به‌جای نقطهٔ رنگی (`pdot`) یک لوگوی ۵۲px (`.proj-logo-lg`)
نشان می‌دهد — `project.logo` یا fallbackِ رنگیِ حرفِ اول از `project.color`.

## بردِ سئو: فیلترِ نوع = دکمه‌های چیپی (این تغییر)
فیلترِ نوعِ تبِ تسک‌ها دیگر دراپ‌داون نیست — **چیپ‌های `.seo-tf`** (لینک، «همهٔ انواع» +
هر نوع) برای جابه‌جاییِ سریع (`projects/detail.html`، CSS در `style.css`). لینک‌ها
`?seo_type=<id>#tasks` می‌سازند تا تبِ تسک‌ها باز بماند (JS: `has('seo_type')` یا
`hash==#tasks`). دکمهٔ **«＋ ستون‌ها» حذف شد**. متنِ راهنمای «برای اینکه در گزارشِ ماهانه
بیایند…» زیرِ سکشنِ بدونِ‌ماه هم حذف شد.

## بردِ سئو: سکشنِ «بدونِ ماهِ گزارش» بالای برد
`ProjectDetailView` علاوه بر سکشن‌های ماه، تسک‌های `report_month__isnull=True` را هم
می‌دهد (`no_month_tasks` نهایتِ ۷ + `no_month_hidden` بقیه + `no_month_total`): انجام‌نشده‌ها
اول (`board_order`)، انجام‌شده‌های بی‌ماه ته. بالای برد رندر می‌شود (کلاسِ `no-month-sec`،
**نه `seo-section`** تا در زنجیرهٔ جابه‌جاییِ سکشن‌ها نیفتد). دکمهٔ «مشاهدهٔ همه»
(`#nm-showall`) tbodyِ `.nm-hidden` را آشکار می‌کند (بدونِ AJAX).

## صفحهٔ لیست (این تغییر) — فیلترِ نوع، اسپارک‌لاین، ستونِ وضعیت، هدرِ ثابت
- **فیلترِ نوعِ پروژه** (`?type=`): پیش‌فرض **`seo`** (فقط پروژه‌های سئو دیده می‌شوند؛
  پروژهٔ شخصیِ خودِ کاربر همیشه، فارغ از نوع)؛ `all` = همه؛ `web`/`tech` هم هست. دراپ‌داونِ
  «نوع» کنارِ «مرتب‌سازی» (هر دو پارامترهای هم را حفظ می‌کنند).
- **ستون‌ها** (کاتالوگ در `core/columns.py`، `core/CLAUDE.md`): «ساعت تخمین»
  (`est_minutes`=Σ estimateِ برنامهٔ بازه) به‌جای ساعتِ کارکرد؛ **اسپارک‌لاینِ روند ۱۴روز**
  (`spark`، هاور=تعدادِ انجام‌شدهٔ آن روز) به‌جای نوارِ پیشرفت؛ یک ستونِ **«وضعیت»**
  (`status_col` = آخرین گزارش + آخرین پرداخت) به‌جای دو ستونِ جدا. مقادیر در
  `ProjectListView.get_context_data` روی هر ردیف ست می‌شوند (یک کوئریِ گروهیِ ۱۴روز).
- **ماه‌های گزارشِ زیرِ پروژه فقط برای پروژه‌های سئو** (`{% if 'seo' in p.types_list %}`)
  و **بدونِ نمودار** (ردیفِ ماه spark ندارد → «—»). کلیک روی ماه → **بردِ سئوِ همان پروژه**
  (`{{ get_absolute_url }}?seo_type=#month-<سال>-<ماه>`) و **اسکرول به سکشنِ همان ماه**
  (`detail.html`: هر `.seo-section` حالا `id="month-<سال>-<ماه>"` دارد + اسکرول/فلَشِ JS).
- **هدرِ ثابتِ جدول** هنگام اسکرول (trap ۳۰ در ریشه: `.proj-card`/`.proj-scroll` روی دسکتاپ
  `overflow:visible`+`backdrop-filter:none` تا sticky به viewport بچسبد).
- «آخرین پرداخت» = آخرین تراکنشِ واریز به پروژه (`Max(transactions__date, deposit__gt=0)`).
