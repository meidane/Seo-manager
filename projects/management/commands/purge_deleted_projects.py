...
منقضی را برای همیشه hard-delete می‌کند. تسک/دسترسی‌ها cascade می‌شوند، اما **فاکتورها
حذف نمی‌شوند** — `Invoice.project` روی `SET_NULL` است، پس فاکتورهای آن پروژه با
`project=None` باقی می‌مانند و همچنان در فهرستِ فاکتورها دیده می‌شوند (دادهٔ مالی نمی‌پرد).
روی cron/زمان‌بند روزانه اجرا کن. `--days` مهلت را override می‌کند؛ `--dry-run` فقط گزارش می‌دهد.
"""
