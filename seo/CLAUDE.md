...
- `management/commands/seed_seo.py` — ۴ نوع تسکِ سئوِ **کاملاً سفارشی** (`builtin_key=''`):
  **انتشار**، **آپدیت**، **رپورتاژ**، **لینک‌سازی** — هرکدام با فیلدِ سفارشیِ خودش
  ...
  - **فیلدهای مشترکِ کلمهٔ کلیدی** (`_KEYWORD_FIELDS` در `seed_seo.py`) بینِ
    ...
    **رپورتاژ ساده‌شد (خواستِ کاربر):** فقط دو فیلدِ **چندخطیِ (`textarea`) خط‌به‌خط جفت‌شونده**
    — «انکر» (`is_keyword_source`) و «لینک» (`is_link_source`). هر خطِ لینک با همان خطِ انکر
    جفت و در `TrackedKeyword(page_url=لینک, keyword=انکر)` ثبت می‌شود (**الگوی ۲** در
    `seo/signals.py`) تا فعالیتِ رپورتاژ در تبِ «کلمات کلیدی»ِ پروژه دیده/ردیابی شود
    (فقط اگر `project.track_keyword_rank`). migrationها: `tasks/0027` (ساده‌سازی)،
    `0028` (بک‌فیلِ کلیدِ خالی)، `0029` (فلگ‌ها + textarea). UI: textareaها با شماره‌گذاریِ
    خودکارِ خطوط (۱-۲-۳، `initLineGutter` در `tasks.js`، کلاس `.cf-ta`).
