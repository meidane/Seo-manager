...
- `management/commands/seed_seo.py` — ۴ نوع تسکِ سئوِ **کاملاً سفارشی** (`builtin_key=''`):
  **انتشار**، **آپدیت**، **رپورتاژ**، **لینک‌سازی** — هرکدام با فیلدِ سفارشیِ خودش
  ...
  - **فیلدهای مشترکِ کلمهٔ کلیدی** (`_KEYWORD_FIELDS` در `seed_seo.py`) بینِ
    ...
    **رپورتاژ ساده‌شد (خواستِ کاربر):** فقط دو فیلد دارد — «انکر» (`text`) و «لینک»
    (`textarea`، چندخطی). کلمه/مترادف/عنوان سئو/تعداد کلمه/لینک صفحه/کلمه کلیدی هدف/
    لینک هدف حذف شدند. برای نوعِ موجود، data-migration `tasks/0027_reportage_fields_simplify`
    فیلدها را بازتنظیم می‌کند (idempotent، فقط نامِ «رپورتاژ»).
